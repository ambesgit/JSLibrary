...
if  new entry is added to the array identified by the name "sample", it will not appear in the html page, to do so you need to 
attach event to the dataHtml() binder.....dataHtml("sample","list", "change"), this assums you create an event called change and listen to it
...
myapp.model("customer","http://localhost:6060/models/customers");      //gets the resource identified by the url and stores it in the $model object
                                          by the "customer" so any part of your application can access this resource using the "customer" name
view()
